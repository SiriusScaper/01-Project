jQuery(document).ready(function($) {
  ...
  slides.on("click", function(event) {
    ...
    if (mq == "desktop" && !slide.hasClass("visible")) {
      ...
      mq == "desktop" &&
      $(event.target).parents(".sub-visible").length == 0 &&
      $(event.target).parents(".sub-slides").length > 0
    ) {
      var newSubSlide = $(event.target)
          .parents(".cd-slider-content")
          .parent("li"),
        direction =
          newSubSlide.prev(".sub-visible").length > 0 ? "next" : "prev";
  ...
  //     slideIndex = !isSubSlide
  //       ? selectedItem.parent("li").index()
  //       : selectedItem
  //           .parents(".sub-nav")
  //           .parent("li")
  //           .index(),
  ...
  $(window).on("resize", function() {
    if (!resizing) {
      !window.requestAnimationFrame
        ? updateOnResize()
        : window.requestAnimationFrame(updateOnResize);
      resizing = true;
    }
  });
  ...
  function updateSubSlide(listItem, string, subSlide) {
    ...
    if (listItem.children(".sub-slides").length > 0) {
      ...
      if (visibleSubSlide.length == 0)
        visibleSubSlide = subSlidesWrapper
          .children("li")
          .eq(0)
          .addClass("sub-visible");
      ...
      if (string == "nav") {
        ...
        var newSubSlide =
          string == "next" ? visibleSubSlide.next() : visibleSubSlide.prev();
      }
  ...
  function updateSlide(string, slide) {
    ...
    var visibleSlide = slides.filter(".visible"),
      marginSlide =
        visibleSlide.find(".cd-slider-content").length > 0
          ? Number(
              visibleSlide
                .find(".cd-slider-content")
                .eq(0)
    ...
    if (string == "nav") {
      ...
      var newSlide =
        string == "next" ? visibleSlide.next() : visibleSlide.prev();
    }

    if (newSlide.length > 0) {
      $(window).off("DOMMouseScroll mousewheel", updateOnScroll);
      var translate = parseInt(
        -newSlide.offset().top + actualTranslate + marginSlide
      );
      translate > 0 && (translate = 0);
      setTransformValue(slideshow.get(0), "translateY", translate + "px");
      visibleSlide.removeClass("visible");
      newSlide.addClass("visible");
      newSlide.find(".sub-visible").length == 0 &&
        newSlide
          .find(".sub-slides")
          .children("li")
          .eq("0")
  ...
  //     var navigationWrapper = $('<ol class="slider-dots"></ol>').appendTo(
  //       container
  //     );
  ...
  function getTranslateValue(element, axis) {
    ...
    if (elementTranslate.indexOf("(") >= 0) {
      ...
      var translateValue =
        axis == "X" ? elementTranslate[4] : elementTranslate[5];
    } else {
